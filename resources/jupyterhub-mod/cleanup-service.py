...
origin_label = "{}={}".format(utils.LABEL_MLRECIPES_ORIGIN, hub_name)
...
def remove_deleted_user_resources(existing_user_names: []):
    ...
    def find_and_remove(docker_client_obj, get_labels, action_callback) -> None:
        """List all resources belonging to `docker_client_obj` which were created by MLRecipes.
        ...
        for resource in resources:
            user_label = get_labels(resource)[utils.LABEL_MLRECIPES_USER]
...
def remove_expired_workspaces(): 
    ...
    for container in hub_containers:
        ...
        if lifetime_timestamp != 0:
            ...
            if difference < 0:
                user_name = unified_container.labels[utils.LABEL_MLRECIPES_USER]
                server_name = unified_container.labels[utils.LABEL_MLRECIPES_SERVER_NAME]
